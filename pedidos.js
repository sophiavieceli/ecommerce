...
const elementoPedido = `<p class="text-xl text-bold my-4">${new Date(pedidoComData.dataPedido).toLocaleDateString('pt-BR', {hour: '2-digit', minute: '2-digit',})}</p>
    ...
    main.innerHTML += elementoPedido; // Só dá problema quando tem ação associada

    for(const idProduto in pedidoComData.pedido){ // in é pra objeto
        desenharProdutoCarrinhoSimples(idProduto, `container-pedidos-${pedidoComData.dataPedido}`, pedidoComData.pedido[idProduto]);
    }
}

function renderizarHistoricoPedidos(){ // of é pra lista
